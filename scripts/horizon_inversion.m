...
for i = 1:length(DCM.field)
    ...
    if ALL
        ...
        % transform the parameters that we fit so the fitting algorithm can
        % work in the space of real numbers
...
% Below is the function used to get the log-likelihood (i.e., accuracy of
% the model)
function L = spm_mdp_L(P,M,U,Y)
    ...
    % Re-transform parameters to their native spaces to plug back into the
    % model
    %--------------------------------------------------------------------------
    params   = M.params; 
